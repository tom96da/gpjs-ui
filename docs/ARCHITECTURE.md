...
| **Frontend framework** | `@gpjs-ui/vue` (first-class, current) / `@gpjs-ui/react` (future, see [Roadmap](./ROADMAP.md#phase-9-react-custom-renderer-future)) | Custom renderer mapping virtual component trees to `gpjs-ui` calls. |
