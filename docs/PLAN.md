...
- [ ] Recover the thrown value from an `EngineError`, whose own `Display`
      flattens every exception to "Exception generated by QuickJS", and split
      it into a message and a stack — a thrown value that isn't an `Error`
      has no stack to report
- [ ] Report an uncaught exception from an app event listener instead of
      swallowing it: an `appError` notification on the dev channel, stderr
      otherwise. `crates/gpjs-ui` takes the reporter as an argument, so it
      stays free of the protocol
