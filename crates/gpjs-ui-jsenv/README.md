...
The host objects installed into a QuickJS realm so ordinary JavaScript can
run in it.
...
on any window or process. That is deliberate: what it installs is
replaceable, either by a third-party implementation or by an application's
own, without any of that reaching the code that draws a UI.
